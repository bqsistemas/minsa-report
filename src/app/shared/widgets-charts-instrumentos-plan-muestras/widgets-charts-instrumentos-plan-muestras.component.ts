...
@Component({
  selector: 'vex-widgets-charts-instrumentos-plan-muestras',
  templateUrl: './widgets-charts-instrumentos-plan-muestras.component.html',
  styleUrls: ['./widgets-charts-instrumentos-plan-muestras.component.scss'],
  animations: [scaleInOutAnimation]
})
export class WidgetsChartsInstrumentosPlanMuestrasComponent implements OnInit, AfterViewInit {
  ...
  fnSearch(instrumento: string = null) {
    ...
    this._muestraService.postResumenAsignacionMuestraNacional(this.idPlanMonitoreo.value, instrumento).then((value: any) => {
      if (!value.success) {
        this.withError = true;
        this.data = [];
  ...
  fnSetOptionesChart() {
    this.chartOptions = {
      ...
      title: {
        text: 'Cantidad de muestras por región y nivel educativo'
      },
      subtitle: {
        text: 'Click en las regiones para ver las cantidades por nivel eduacativo.'
      },
      accessibility: {
        announceNewData: {
          enabled: true
  ...
  fnGetData(data: DasboardAsignacionMuestra[]) {
    data.forEach((item) => {
      if (!this.dataRegiones.some(x => x.drilldown === item.dre)) {
        this.dataRegiones.push({
          name: item.dreDescripcion,
          y: 0,
          drilldown: item.dre
        });
      }
    ...
    this.dataRegiones.forEach((region) => {
      region.y = data.filter(x => x.dre === region.drilldown).reduce((prev, cur) => {
        return prev + cur.cantidad;
      }, 0);
    });
      ...
      this.dataDrillDownByRegiones.push(
        {
          name: region.name,
          id: region.drilldown,
          data: data.filter(x => x.dre === region.drilldown)
