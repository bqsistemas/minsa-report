...
namespace Care.Minsa.DPVIH.Tablero.Application.CommandsHandlers
{
    public class PeriodoHandler :
        IRequestHandler<ListarPeriodoCommand, List<PeriodoDto>>
    {
        private readonly IFiltrosReporteQuery _filtrosReporteQuery;
        public PeriodoHandler(IFiltrosReporteQuery filtrosReporteQuery)
        {
            _filtrosReporteQuery = filtrosReporteQuery;
        }

        public Task<List<PeriodoDto>> Handle(ListarPeriodoCommand request, CancellationToken cancellationToken)
        {
            return _filtrosReporteQuery.GetPeriodo();
        }
    }
}
